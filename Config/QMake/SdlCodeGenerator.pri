...
SDL_GENERATOR_OUT_SUBFOLDER = $$OUT_PWD/../../../AuxInclude/Qt$${QT_MAJOR_VERSION}_$$COMPILER_CODE/GeneratedFiles/$$TARGET/SDL

SDL_GENERATOR_CACHE_FILE_PATH += $$absolute_path($$PWD/../../AuxInclude/Qt$${QT_MAJOR_VERSION}_$$COMPILER_CODE/SDL_CACHE/cache.jcache)
...
win32{
	...
	SDL_GENERATOR_CACHE_FILE_PATH ~= s,/,\\,g
}
...
defineReplace(CreateSdlTarget) {
	...
	SDL_EXTRA_INCLUDES_DIRECTIVE =
	# look for paths in INCLUDEPATH if IMT_GLOBAL_SDL_INCLUDEPATH is not defined
	isEmpty(IMT_GLOBAL_SDL_INCLUDEPATH) {
		for (SDL_EXTRA_PATH, INCLUDEPATH) {
			SDL_EXTRA_INCLUDES_DIRECTIVE += -H$$absolute_path($${SDL_EXTRA_PATH})
		}
	}
	# and look for paths in IMT_GLOBAL_SDL_INCLUDEPATH otherwise
	!isEmpty(IMT_GLOBAL_SDL_INCLUDEPATH) {
		$$unique(IMT_GLOBAL_SDL_INCLUDEPATH)
		for (SDL_EXTRA_PATH, IMT_GLOBAL_SDL_INCLUDEPATH) {
			SDL_EXTRA_INCLUDES_DIRECTIVE += -H$${SDL_EXTRA_PATH}
		}
	}

	SDL_EXTRA_MODULES_DIRECTIVE =
	# look for paths in INCLUDEPATH if IMT_GLOBAL_SDL_MODULES_PATHS is not defined
	isEmpty(IMT_GLOBAL_SDL_MODULES_PATHS) {
		for (SDL_EXTRA_PATH, INCLUDEPATH) {
			SDL_EXTRA_MODULES_DIRECTIVE += -E$$absolute_path($${SDL_EXTRA_PATH})
		}
	}
	# and look for paths in IMT_GLOBAL_SDL_MODULES_PATHS otherwise
	!isEmpty(IMT_GLOBAL_SDL_MODULES_PATHS) {
		$$unique(IMT_GLOBAL_SDL_MODULES_PATHS)
		for (SDL_EXTRA_PATH, IMT_GLOBAL_SDL_MODULES_PATHS) {
			SDL_EXTRA_MODULES_DIRECTIVE += -E$${SDL_EXTRA_PATH}
		}
	}


	SDL_GENERATED_FILES = $$system($${SDL_GENERATOR_COMMAND_BASE} -DS $${SDL_GENERATOR_SCHEME_PATH} --CPP --GQL -Psdl --auto-link=2 --GG=2 $$SDL_EXTRA_INCLUDES_DIRECTIVE $$SDL_EXTRA_MODULES_DIRECTIVE)
	...
	$${SDL_COMPILER_NAME}.commands = $${SDL_GENERATOR_COMMAND_BASE} -GS $${SDL_GENERATOR_SCHEME_PATH} --CPP --GQL -Psdl --auto-link=2 --GG=2 $$SDL_EXTRA_INCLUDES_DIRECTIVE $$SDL_EXTRA_MODULES_DIRECTIVE --generate-module
