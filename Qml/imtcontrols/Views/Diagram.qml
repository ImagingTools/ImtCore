...
Item {
	...
	Item{
		...
		anchors.bottom: barsSpace.bottom;
		anchors.bottomMargin: barsList.height/(diagram.maxValue - diagram.minValue) * (diagram.maxValue - diagram.minValue)/2
		anchors.left: barsSpace.left;
		anchors.right: barsSpace.right;

		height: 2;
		visible: diagram.hasMiddleLine && barsList.count;
		property string color: diagram.middleLineColor;
		onVisibleChanged: {
			if(visible){
				canvas.requestPaint();
			}
