...
Canvas {
    ...
    onPaint: {
        ...
        var ctx = tableDelegateContainer.getContext('2d');
        ctx.reset();
        ctx.clearRect(0, 0, tableDelegateContainer.width, tableDelegateContainer.height);

        let containsMouse = tableDelegateContainer.tableItem.hoverEnabled && ma.containsMouse;
        let rowCount = tableDelegateContainer.tableItem ? tableDelegateContainer.tableItem.elementsCount : 0;

        // if (tableDelegateContainer.tableItem.enableAlternating || tableDelegateContainer.tableItem.selectable){

            let color = (containsMouse || tableDelegateContainer.selected) ? Style.selectedColor :
                    tableDelegateContainer.tableItem.enableAlternating ? tableDelegateContainer.tableItem.alternatingColor : 'white';
            ctx.fillStyle = color
            ctx.fillRect(x, y, width, height)
        // }

        ctx.beginPath()
        ctx.strokeStyle =  tableDelegateContainer.tableItem.borderColorHorizontal // "red" //tableDelegateContainer.borderColorHorizontal
        // ctx.fillStyle = "red" //tableDelegateContainer.selected ? Style.selectedColor : "transparent";

        ctx.moveTo(0, 0);
        ctx.lineWidth = (tableDelegateContainer.rowIndex > 0 && tableDelegateContainer.rowIndex < rowCount) ? tableDelegateContainer.horizontalBorderSize :
                                                                                                                                      (tableDelegateContainer.rowIndex == 0 && tableDelegateContainer.visibleTopBorderFirst) ? tableDelegateContainer.horizontalBorderSize : 0;
        ctx.lineTo(tableDelegateContainer.width, 0);

        ctx.lineWidth =  tableDelegateContainer.visibleBottomBorderLast  ? tableDelegateContainer.horizontalBorderSize : 0;

        ctx.moveTo(0, tableDelegateContainer.height);
        ctx.lineTo(tableDelegateContainer.width, tableDelegateContainer.height);

        ctx.stroke();
        let xCell = 0;
        ctx.strokeStyle = tableDelegateContainer.tableItem.borderColorVertical //"red" //tableDelegateContainer.borderColorVertical;
        let fontPixelSize = tableDelegateContainer.tableItem.emptyDecorCell ? Style.fontSize_common : tableDelegateContainer.tableItem.cellDecorator.isValidData("FontSize", columnIndex) ? tableDelegateContainer.tableItem.cellDecorator.getData("FontSize", columnIndex) : Style.fontSize_common
        ctx.font = '' + fontPixelSize + "px " + Style.fontFamily

        for (let columnIndex = 0; columnIndex < tableDelegateContainer.columnCount; columnIndex++){
// draw top border
            // let lineWidth = (tableDelegateContainer.rowIndex > 0 && tableDelegateContainer.rowIndex < tableDelegateContainer.rowCount) ? tableDelegateContainer.horizontalBorderSize :
                                                                                                                                                   // (tableDelegateContainer.rowIndex == 0 && tableDelegateContainer.visibleTopBorderFirst) ? tableDelegateContainer.horizontalBorderSize : 0;


            let defaultWidth = tableDelegateContainer.columnCount == 0 ? 0 : tableDelegateContainer.width/tableDelegateContainer.columnCount;
            let widthFromModel = tableDelegateContainer.tableItem.widthDecoratorDynamic.isValidData("Width", columnIndex) ?
                        tableDelegateContainer.tableItem.widthDecoratorDynamic.getData("Width", columnIndex) : -1;

            let cellWidth = 0;
            if(!tableDelegateContainer.tableItem.widthDecoratorDynamic.getItemsCount()){
            ...
            let lineWidth = tableDelegateContainer.isRightBorder ? tableDelegateContainer.verticalBorderSize * tableDelegateContainer.visibleLeftBorderFirst * (columnIndex == 0)
                                                                 : tableDelegateContainer.visibleLeftBorderFirst ? tableDelegateContainer.verticalBorderSize : columnIndex > 0 ? tableDelegateContainer.verticalBorderSize : 0;
            // lineWidth = 1 // for test

            if (lineWidth > 0){
                ctx.lineWidth = lineWidth
                ctx.moveTo(xCell, 0);
                ctx.lineTo(xCell, tableDelegateContainer.height)
            }

            lineWidth = !tableDelegateContainer.isRightBorder ?
                            tableDelegateContainer.verticalBorderSize * tableDelegateContainer.visibleRightBorderLast  * (columnIndex == (tableDelegateContainer.columnCount -1)) * (tableDelegateContainer.columnCount > 0) :
                            tableDelegateContainer.visibleRightBorderLast ? tableDelegateContainer.verticalBorderSize  :
                            tableDelegateContainer.verticalBorderSize * (columnIndex < (tableDelegateContainer.columnCount -1));

            xCell += cellWidth;

             // lineWidth = 2 // for test
            ...
            ctx.fillStyle = tableDelegateContainer.tableItem.emptyDecorCell ? Style.textColor : tableDelegateContainer.tableItem.cellDecorator.isValidData("FontColor", columnIndex) ? tableDelegateContainer.tableItem.cellDecorator.getData("FontColor", columnIndex) : Style.textColor

            drawCell(ctx, xCell, 15, cellWidth, tableDelegateContainer.height, columnIndex)
    ...
    function drawCell(ctx, x, y, cellWidth, cellHeight, columnIndex){
        if(tableDelegateContainer.tableItem !==null && tableDelegateContainer.dataModel != null){
            let val
            let key = tableDelegateContainer.tableItem.headers.getData("Id", columnIndex)
            if ("item" in tableDelegateContainer.dataModel){
                val = tableDelegateContainer.dataModel.item[key]
            }
            else {
                val = tableDelegateContainer.dataModel[key];
            }
