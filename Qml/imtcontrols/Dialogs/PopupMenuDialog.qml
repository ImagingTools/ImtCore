...
ControlBase {
	...
	property Component delegate: PopupMenuDelegate {
		...
		onClicked: {
			...
			popupMenuContainer.finished(resultId, model.index);
		}
	...
	function onReturn(){
		if(popupMenuContainer.selectedIndex >= 0){
			var id = ""
			if (popupMenuContainer.model.containsKey("id", popupMenuContainer.selectedIndex)){
				id = popupMenuContainer.model.getData("id", popupMenuContainer.selectedIndex);
			}
			if (popupMenuContainer.model.containsKey("Id", popupMenuContainer.selectedIndex)){
				id = popupMenuContainer.model.getData("Id", popupMenuContainer.selectedIndex);
			}
