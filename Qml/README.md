...
| Папка | Назначение |
|---|---|
| `Pages/` | Страницы верхнего уровня и контейнеры-страницы (`*Page`, `*PageView`) |
| `Views/` | Визуальные представления данных (`*View`, панели-виджеты, бейджи, карточки) |
| `Panels/` | Панели (боковые/списочные области) |
| `Dialogs/` | Диалоги и всплывающие окна (`*Dialog`, `*Popup`) |
| `Editors/` | Редакторы сущностей (`*Editor`) |
| `Inputs/` | Элементы ввода (поля, комбобоксы, пикеры) |
| `Delegates/` | Делегаты элементов списков/таблиц (`*Delegate`) |
| `Controllers/` | Контроллеры и менеджеры (невизуальная координация) |
| `Providers/` | Провайдеры данных (`*Provider`) |
| `Services/` | Сервисы (`*Service`) |
| `Api/` | Сетевой слой: запросы, отправители, клиенты, сокеты, подписки |
| `Models/` | Модели, кэши коллекций, фильтры, валидаторы, структуры |
| `Base/` | Базовые компоненты и стили |

Для `imt3dgui` используется предметная группировка Qt Quick 3D
(`Cameras/`, `Lights/`, `Materials/`, `Geometry/`, `Particles/` и т. д.).
