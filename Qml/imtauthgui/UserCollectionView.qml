...
RemoteCollectionView {
	...
	commandsDelegateComp: Component {DocCollectionViewDelegate {
			...
			Component.onCompleted: {
				...
				addDocumentView("User", "UserEditor", userDocumentComp, userControllerComp)
			}

			onDocumentManagerChanged: {
				if (documentManager){
					userCollectionViewContainer.documentManager = documentManager
					documentManager.backend.registerDocumentDataController("User", documentDataControllerComp)
				}
			}
	...
	Component {
		id: userControllerComp

		InProcessDocumentRepresentationController {
			backend: MainDocumentService.getDocumentService("Administration/Users").backend
		}
	}
