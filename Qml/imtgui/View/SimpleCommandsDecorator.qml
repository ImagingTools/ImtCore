...
Item {
	...
	function setCommandVisible(commandId, visible){
		...
		for (let i = 0; i < container.commandModel.getItemsCount(); i++){
			...
			if (id === commandId){
				let currentIsVisible = container.commandModel.getData("visible", visible, i);
				if (currentIsVisible !== visible){
	...
	function isCommandVisible(commandId){
		if(!container.commandModel){
			return false;
		}
		...
		for (let i = 0; i < container.commandModel.getItemsCount(); i++){
			let id = container.commandModel.getData("id", i);
			if (id === commandId){
				let currentIsVisible = container.commandModel.getData("visible", i);
	...
	function isCommandIsEnabled(commandId){
		if(!container.commandModel){
			return false;
		}
