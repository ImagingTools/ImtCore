		...
		if (!paramIds.isEmpty()){
			...
			for (int i = 0; i < idsList.size(); i++){
				...
				if (key == "VerboseFilter" && value == "false"){
					...
					filterQuery += QStringLiteral("json_extract(\"Document\",'$.Category') != 0");
				}
				else if (key == "InfoFilter" && value == "false"){
					if (!filterQuery.isEmpty()){
						filterQuery += " AND ";
					}
					filterQuery += QStringLiteral("json_extract(\"Document\",'$.Category') != 1");
				}
				else if (key == "WarningFilter" && value == "false"){
					if (!filterQuery.isEmpty()){
						filterQuery += " AND ";
					}
					filterQuery += QStringLiteral("json_extract(\"Document\",'$.Category') != 2");
				}
				else if (key == "ErrorFilter" && value == "false"){
					if (!filterQuery.isEmpty()){
						filterQuery += " AND ";
					}
					filterQuery += QStringLiteral("json_extract(\"Document\",'$.Category') != 3");
				}
				else if (key == "CriticalFilter" && value == "false"){
					if (!filterQuery.isEmpty()){
						filterQuery += " AND ";
					}
					filterQuery += QStringLiteral("json_extract(\"Document\",'$.Category') != 4");
				}
			}
		}
	}
		...
		if (!paramIds.isEmpty()){
			...
			for (int i = 0; i < idsList.size(); i++){
				...
				if (value == "false"){
					...
					filterQuery += QStringLiteral("json_extract(\"Document\",'$.Source') != '%1'").arg(key);
				}
			}
		}
	}
