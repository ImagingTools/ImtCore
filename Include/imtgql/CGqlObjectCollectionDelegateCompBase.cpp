...
{
	if (m_clientCompPtr.IsValid()){
		istd::TDelPtr<IGqlRequest> requestPtr = CreateSetNodeMetaInfoRequest(
					nodeId,
					metaInfo,
					operationContextPtr);
		if (requestPtr.IsValid()){
...
{
	if (m_clientCompPtr.IsValid()){
		istd::TDelPtr<IGqlRequest> requestPtr = CreateMoveNodeRequest(
					nodeId,
					parentNodeId,
					operationContextPtr);
		if (requestPtr.IsValid()){
...
{
	if (m_clientCompPtr.IsValid()){
		istd::TDelPtr<IGqlRequest> requestPtr = CreateRemoveNodeRequest(
					nodeId,
					operationContextPtr);
		if (requestPtr.IsValid()){
			istd::TDelPtr<IGqlPrimitiveTypeResponse> responsePtr;
...
bool CGqlObjectCollectionDelegateCompBase::AddObjectToNode(const QByteArray& objectId, const QByteArray& nodeId, const imtbase::IOperationContext* operationContextPtr)
{
	if (m_clientCompPtr.IsValid()){
		...
		if (GetObjectInfo(objectId, info)){
			istd::TDelPtr<IGqlRequest> requestPtr = CreateAddObjectToNodeRequest(
						objectId,
						nodeId,
						info.version,
						operationContextPtr);
...
bool CGqlObjectCollectionDelegateCompBase::MoveObjectToNode(const QByteArray& objectId, const QByteArray& nodeId, const QByteArray& newNodeId, const imtbase::IOperationContext* operationContextPtr)
{
	if (m_clientCompPtr.IsValid()){
		IGqlStructuredCollectionResponse::ObjectInfo info;
		if (GetObjectInfo(objectId, info)){
			istd::TDelPtr<IGqlRequest> requestPtr = CreateMoveObjectToNodeRequest(
						objectId,
						nodeId,
						newNodeId,
						info.version,
...
bool CGqlObjectCollectionDelegateCompBase::RemoveObjectFromNode(const QByteArray& objectId, const QByteArray& nodeId, const imtbase::IOperationContext* operationContextPtr) const
{
	if (m_clientCompPtr.IsValid()){
		IGqlStructuredCollectionResponse::ObjectInfo info;
		if (GetObjectInfo(objectId, info)){
			istd::TDelPtr<IGqlRequest> requestPtr = CreateRemoveObjectFromNodeRequest(
						objectId,
						nodeId,
						info.version,
						operationContextPtr);
...
bool CGqlObjectCollectionDelegateCompBase::SetObjectName(const QByteArray& objectId, const QString& name, const imtbase::IOperationContext* operationContextPtr) const
{
	if (m_clientCompPtr.IsValid()){
		IGqlStructuredCollectionResponse::ObjectInfo info;
		if (GetObjectInfo(objectId, info)){
			istd::TDelPtr<IGqlRequest> requestPtr = CreateSetObjectNameRequest(objectId, name, info.version);
			istd::TDelPtr<IGqlPrimitiveTypeResponse> responsePtr;
			responsePtr.SetCastedOrRemove(CreateResponse(*requestPtr));
			if (responsePtr.IsValid()){
				if (m_clientCompPtr->SendRequest(*requestPtr, *responsePtr)){
...
bool CGqlObjectCollectionDelegateCompBase::SetObjectDescription(const QByteArray& objectId, const QString& description, const imtbase::IOperationContext* operationContextPtr) const
{
	if (m_clientCompPtr.IsValid()){
		IGqlStructuredCollectionResponse::ObjectInfo info;
		if (GetObjectInfo(objectId, info)){
			istd::TDelPtr<IGqlRequest> requestPtr = CreateSetObjectDescriptionRequest(objectId, description, info.version);
			istd::TDelPtr<IGqlPrimitiveTypeResponse> responsePtr;
			responsePtr.SetCastedOrRemove(CreateResponse(*requestPtr));
			if (responsePtr.IsValid()){
				if (m_clientCompPtr->SendRequest(*requestPtr, *responsePtr)){
...
bool CGqlObjectCollectionDelegateCompBase::SetObjectMetaInfo(const QByteArray& objectId, const idoc::IDocumentMetaInfo& dataMetaInfo, const imtbase::IOperationContext* operationContextPtr) const
{
	if (m_clientCompPtr.IsValid()){
		IGqlStructuredCollectionResponse::ObjectInfo info;
		if (GetObjectInfo(objectId, info)){
			istd::TDelPtr<IGqlRequest> requestPtr = CreateSetObjectMetaInfoRequest(
						objectId,
						dataMetaInfo,
						info.version,
						operationContextPtr);
...
IGqlRequest* CGqlObjectCollectionDelegateCompBase::CreateGetNodeInfoRequest(const QByteArray& nodeId) const
{
	return nullptr;
}


IGqlRequest* CGqlObjectCollectionDelegateCompBase::CreateGetObjectInfoRequest(const QByteArray& objectId) const
{
	return nullptr;
}


IGqlRequest* CGqlObjectCollectionDelegateCompBase::CreateInsertNodeRequest(const QString& name, const QString& description, const QByteArray& proposedNodeId, const QByteArray& parentNodeId, const idoc::IDocumentMetaInfo* metaInfoPtr, const imtbase::IOperationContext* operationContextPtr) const
{
	return nullptr;
}


IGqlRequest* CGqlObjectCollectionDelegateCompBase::CreateSetNodeNameRequest(const QByteArray& nodeId, const QString& name, const imtbase::IOperationContext* operationContextPtr) const
{
	return nullptr;
}


IGqlRequest* CGqlObjectCollectionDelegateCompBase::CreateSetNodeDescriptionRequest(const QByteArray& nodeId, const QString& description, const imtbase::IOperationContext* operationContextPtr) const
{
	return nullptr;
}


IGqlRequest* CGqlObjectCollectionDelegateCompBase::CreateSetNodeMetaInfoRequest(const QByteArray& nodeId, const idoc::IDocumentMetaInfo& metaInfo, const imtbase::IOperationContext* operationContextPtr) const
{
	return nullptr;
}


IGqlRequest* CGqlObjectCollectionDelegateCompBase::CreateMoveNodeRequest(const QByteArray& nodeId, const QByteArray& parentNodeId, const imtbase::IOperationContext* operationContextPtr) const
{
	return nullptr;
}


IGqlRequest* CGqlObjectCollectionDelegateCompBase::CreateRemoveNodeRequest(const QByteArray& nodeId, const imtbase::IOperationContext* operationContextPtr) const
{
	return nullptr;
}


IGqlRequest* CGqlObjectCollectionDelegateCompBase::CreateAddObjectToNodeRequest(const QByteArray& objectId, const QByteArray& nodeId, int clientVersion, const imtbase::IOperationContext* operationContextPtr)
{
	return nullptr;
}


IGqlRequest* CGqlObjectCollectionDelegateCompBase::CreateMoveObjectToNodeRequest(const QByteArray& objectId, const QByteArray& nodeId, const QByteArray& newNodeId, int clientVersion, const imtbase::IOperationContext* operationContextPtr)
{
	return nullptr;
}


IGqlRequest* CGqlObjectCollectionDelegateCompBase::CreateRemoveObjectFromNodeRequest(const QByteArray& objectId, const QByteArray& nodeId, int clientVersion, const imtbase::IOperationContext* operationContextPtr) const
{
	return nullptr;
}


IGqlRequest* CGqlObjectCollectionDelegateCompBase::CreateInsertObjectRequest(const QByteArray& typeId, const QString& name, const QString& description, const istd::IChangeable* objectPtr, const QString& uploadUrl, const QByteArray& proposedObjectId, const QByteArray& nodeId, const idoc::IDocumentMetaInfo* dataMetaInfoPtr, const idoc::IDocumentMetaInfo* collectionItemMetaInfoPtr, const imtbase::IOperationContext* operationContextPtr) const
{
	return nullptr;
}


IGqlRequest* CGqlObjectCollectionDelegateCompBase::CreateSetObjectNameRequest(const QByteArray& objectId, const QString& name, int clientVersion, const imtbase::IOperationContext* operationContextPtr) const
{
	return nullptr;
}


IGqlRequest* CGqlObjectCollectionDelegateCompBase::CreateSetObjectDescriptionRequest(const QByteArray& objectId, const QString& description, int clientVersion, const imtbase::IOperationContext* operationContextPtr) const
{
	return nullptr;
}


IGqlRequest* CGqlObjectCollectionDelegateCompBase::CreateSetObjectMetaInfoRequest(const QByteArray& objectId, const idoc::IDocumentMetaInfo& metaInfo, int clientVersion, const imtbase::IOperationContext* operationContextPtr) const
{
	return nullptr;
}


IGqlRequest* CGqlObjectCollectionDelegateCompBase::CreateGetObjectRequest(const QByteArray& objectId) const
{
	return nullptr;
}
