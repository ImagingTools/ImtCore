...
bool CSdlClassJsonModificatorComp::ProcessHeaderClassFile(const imtsdl::CSdlType& sdlType)
{
	...
	ofStream << QStringLiteral("\t[[nodiscard]] static bool OptReadFromJsonObject(C");
...
bool CSdlClassJsonModificatorComp::ProcessSourceClassFile(const imtsdl::CSdlType& sdlType)
{
	...
	ofStream << QStringLiteral("bool C");
	ofStream << sdlType.GetName();
	ofStream << QStringLiteral("::OptReadFromJsonObject(C");
	ofStream << sdlType.GetName();
	ofStream << QStringLiteral("& object, const QJsonObject& jsonObject)\n{");
	FeedStream(ofStream, 1, false);
...
void CSdlClassJsonModificatorComp::AddFieldWriteToJsonCode(QTextStream& stream, const imtsdl::CSdlField& field, bool optional)
{
	...
	if (!optional && field.IsRequired()){
		...
		if (isEnum){
			...
			stream << QStringLiteral("if (const auto* val = std::get_if<0>(m_") + GetDecapitalizedValue(field.GetId()) + QStringLiteral(")){\n\t");
			stream << QStringLiteral("\tjsonObject[\"") << field.GetId() << QStringLiteral("\"] = *val;\n\t");
			stream << QStringLiteral("}\n\t");

			for (int i = 1; i < foundUnion.GetTypes().size(); ++i){
				stream << QStringLiteral("else if (const auto* val = std::get_if<%1>(m_").arg(i) + GetDecapitalizedValue(field.GetId()) + QStringLiteral(")){\n\t");
				stream << QStringLiteral("\tjsonObject[\"") << field.GetId() << QStringLiteral("\"] = *val;\n\t");
				stream << QStringLiteral("}\n\t");
			}
		}
...
void CSdlClassJsonModificatorComp::AddFieldReadFromJsonCode(QTextStream& stream, const imtsdl::CSdlField& field, bool optional)
{
	...
	if (!optional && field.IsRequired()){
		...
		if (isEnum){
			...
			WriteEnumConversionFromString(stream, foundEnum, enumSourceVarName, enumConvertedVarName, m_originalSchemaNamespaceCompPtr->GetText(), 2);
