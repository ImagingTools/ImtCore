...
//	 control.inputEnter();
	/*---------------Сальдо---------------------*/
	 ...
	 curTextStyle.m_font.setBold(true);
	 control.setTextStyle(&curTextStyle);
	 control.setText("Сальдо общее на " + QDateTime::currentDateTime().toString("yyyy.MM.dd"));
	 control.inputEnter();

	 curTextStyle.setDefault();
	 control.setTextStyle(&curTextStyle);

	 EjTableBlock *table = new EjTableBlock(17,6, ejDoc, control.activeIndex);
	 ...
	 curCell->setText("Филиал",&control);
