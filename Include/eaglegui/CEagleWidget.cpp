...
const float CEagleWidget::s_verticalAngle = 45.0;
const float CEagleWidget::s_nearPlane = 0.1;
const float CEagleWidget::s_farPlane = 100.0;
const float CEagleWidget::s_defaultLineWidth = 1.0f;
const QVector3D CEagleWidget::s_defaultCameraPosition = { 0.0, 0.0, 5.0 };
const QVector3D CEagleWidget::s_lightPosition(1.2, 1.0, 2.0);
const QVector3D CEagleWidget::s_lightColor(1.0, 1.0, 1.0);


// public methods

CEagleWidget::CEagleWidget(QWidget *parent)
...
void CEagleWidget::SetSceneEventHandler(ISceneEventHandler* handlerPtr)
{
	m_eventHandler = handlerPtr;
}


void CEagleWidget::UnsetSceneEventHandler()
{
	m_eventHandler = nullptr;
}


eagle::IScene3dCamera *CEagleWidget::GetCamera()
{
	return &m_camera;
}


eagle::IScene3d* CEagleWidget::GetScene()
{
	return &m_scene;
}


void CEagleWidget::ZoomIn()
{
...
void CEagleWidget::ShowGrid(bool show)
{
	if (m_eventHandler){
...
void CEagleWidget::ShowAxis(bool show)
{
	if (m_eventHandler){
...
bool CEagleWidget::GetRenderHint(RenderHint renderHint) const
{
	return m_renderHints & renderHint;
}


void CEagleWidget::SetRenderHint(RenderHint renderHint, bool on)
{
	if (on){
...
void CEagleWidget::SetCameraView(CEagleWidget::ViewDirection viewDirection, bool animated)
{
...
void CEagleWidget::SetBoxSelection(bool on)
{
	SetSelection(SM_BOX, on);
}


void CEagleWidget::SetCircleSelection(bool on)
{
	SetSelection(SM_CIRCLE, on);
}


void CEagleWidget::ClearSelection()
{
	if (m_eventHandler){
...
void CEagleWidget::InvertSelection()
{
	if (m_eventHandler){
...
void CEagleWidget::DeleteSelection()
{
	if (m_eventHandler){
...
void CEagleWidget::initializeGL()
{
...
void CEagleWidget::resizeGL(int w, int h)
{
...
void CEagleWidget::mousePressEvent(QMouseEvent* e)
{
	m_cameraRotationAnimation.stop();
	m_cameraPositionAnimation.stop();
...
void CEagleWidget::mouseReleaseEvent(QMouseEvent* /*e*/)
{
	m_selectionRect.setRect(0.0, 0.0, 0.0, 0.0);
}


void CEagleWidget::mouseMoveEvent(QMouseEvent* e)
{
	if (m_viewMode == VM_SHOW){
...
void CEagleWidget::closeEvent(QCloseEvent* eventPtr)
{
...
void CEagleWidget::wheelEvent(QWheelEvent* eventPtr)
{
	if (eventPtr->delta() > 0){
		OnZoomIn();
	}
...
void CEagleWidget::OnZoomIn()
{
	ZoomIn();
}


void CEagleWidget::OnZoomOut()
{
	ZoomOut();
}


void CEagleWidget::OnInternalTimer()
{
	QElapsedTimer timer;
	timer.start();
...
void CEagleWidget::OnCameraRotationAnimation(const QVariant& value)
{
...
void CEagleWidget::OnCameraPositionAnimation(const QVariant& value)
{
...
void CEagleWidget::Paint(QPainter& painter)
{
...
void CEagleWidget::PaintSelection(QPainter& painter)
{
...
void CEagleWidget::MouseMoveEventShow(QMouseEvent& e)
{
...
void CEagleWidget::MouseMoveEventSelection(QMouseEvent& e)
{
...
void CEagleWidget::SetSelection(SelectionMode selectionMode, bool on)
{
	if (on){
...
void CEagleWidget::SetGlUniformValues()
{
...
QMatrix4x4 CEagleWidget::GetProjectionMatrix() const
{
