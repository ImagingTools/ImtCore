...
// Eagle includes
#include <eagle/CTrackballCamera.h>
#include <eagle/CScene3d.h>
...
class CEagleWidget: public QOpenGLWidget, protected QOpenGLFunctions
{
	Q_OBJECT
...
public:
	...
	CEagleWidget(QWidget *parent = 0);
	~CEagleWidget();
	
	void SetSceneEventHandler(ISceneEventHandler* handlerPtr);
	void UnsetSceneEventHandler();
	eagle::IScene3dCamera* GetCamera();
	eagle::IScene3d* GetScene();
	void ZoomIn();
	void ZoomOut();
...
private:
	...
	eaglegui::ISceneEventHandler* m_eventHandler;
	eagle::CTrackballCamera m_camera;
	eagle::CScene3d m_scene;
	QTimer m_timer;
