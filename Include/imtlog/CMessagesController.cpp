...
void CMessagesController::Writer::run()
{
	...
	while (!m_parent.m_writingQueue.isEmpty()){
		...
		if (istd::CSystem::EnsurePathExists(tempDir)){
			do{
				...
				if (!containerPtr->Serialize(xmlArchive)){
					m_parent.SendErrorMessage(
								0, QObject::tr("Unable to serialize message history container \"%1\". Event container skipped")
								.arg(containerPath));
				...
				if (m_compressorPtr != nullptr){
					if (!m_compressorPtr->CompressFile(containerPath, archivePath)){
						m_parent.SendErrorMessage(
									0, QObject::tr("Unable to compress message history container \"%1\". Event container skipped")
									.arg(archivePath));
				...
				if (!istd::CSystem::FileCopy(archivePath, outPath, true)){
					m_parent.SendErrorMessage(
								0, QObject::tr("Unable to copy history archive to repository. Event container skipped")
								.arg(archivePath));
			...
			m_parent.SendErrorMessage(0, QObject::tr("Cannot create temporary folder. Event container skipped"));
		}
	}
}
