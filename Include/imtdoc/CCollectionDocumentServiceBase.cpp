...
bool CCollectionDocumentServiceBase::HasDocumentNameProvider(const QByteArray& typeId) const
{
	return GetDocumentNameProvider(typeId) != nullptr;
}


bool CCollectionDocumentServiceBase::ValidateDocumentData(
...
QList<imtdoc::IDocumentServiceEventHandler*> CCollectionDocumentServiceBase::GetDocumentServiceEventHandlers() const
{
...
istd::IChangeableUniquePtr CCollectionDocumentServiceBase::CreateObject(const QByteArray& typeId) const
{
	...
	Q_ASSERT_X(false, "CCollectionDocumentServiceBase::CreateObject", qPrintable(QString("Factory not found for the type: '%1'").arg(qPrintable(typeId))));

	return nullptr;
}


idoc::IUndoManagerUniquePtr CCollectionDocumentServiceBase::CreateUndoManager() const
{
	if (m_undoManagerFactPtr.IsValid()){
		return m_undoManagerFactPtr.CreateInstance();
	}

	Q_ASSERT_X(false, "CCollectionDocumentServiceBase::CreateUndoManager", qPrintable(QString("Factory not found")));
...
int CCollectionDocumentServiceBase::GetObjectFactoryIndex(const QByteArray& typeId) const
{
...
const imtdoc::IDocumentNameProvider* CCollectionDocumentServiceBase::GetDocumentNameProvider(const QByteArray& typeId) const
{
	int index = GetObjectFactoryIndex(typeId);
...
const imtdoc::IDocumentValidator* CCollectionDocumentServiceBase::GetDocumentValidator(const QByteArray& typeId) const
{
	int index = GetObjectFactoryIndex(typeId);
