...
QByteArray CCollectionDocumentServiceComp::CreateNewDocument(
...
QByteArray CCollectionDocumentServiceComp::OpenDocument(const QByteArray& userId, const QUrl& url)
{
	QByteArray retVal;
...
IDocumentService::OperationStatus CCollectionDocumentServiceComp::SetDocumentName(const QByteArray& userId, const QByteArray& documentId, const QString& documentName)
{
	QMutexLocker locker(&m_mutex);
...
IDocumentService::OperationStatus CCollectionDocumentServiceComp::SaveDocument(
	const QByteArray& userId,
...
IDocumentService::OperationStatus CCollectionDocumentServiceComp::CloseDocument(
...
bool CCollectionDocumentServiceComp::HasDocumentNameProvider(const QByteArray& typeId) const
{
	return GetDocumentNameProvider(typeId) != nullptr;
}


bool CCollectionDocumentServiceComp::ValidateDocumentData(
...
QList<imtdoc::IDocumentServiceEventHandler*> CCollectionDocumentServiceComp::GetDocumentServiceEventHandlers() const
{
...
istd::IChangeableUniquePtr CCollectionDocumentServiceComp::CreateObject(const QByteArray& typeId) const
{
	...
	Q_ASSERT_X(false, "CCollectionDocumentServiceComp::CreateObject", qPrintable(QString("Factory not found for the type: '%1'").arg(qPrintable(typeId))));

	return nullptr;
}


idoc::IUndoManagerUniquePtr CCollectionDocumentServiceComp::CreateUndoManager() const
{
	if (m_undoManagerFactPtr.IsValid()){
		return m_undoManagerFactPtr.CreateInstance();
	}

	Q_ASSERT_X(false, "CCollectionDocumentServiceComp::CreateUndoManager", qPrintable(QString("Factory not found")));
...
int CCollectionDocumentServiceComp::GetObjectFactoryIndex(const QByteArray& typeId) const
{
...
const imtdoc::IDocumentNameProvider* CCollectionDocumentServiceComp::GetDocumentNameProvider(const QByteArray& typeId) const
{
	int index = GetObjectFactoryIndex(typeId);
...
const imtdoc::IDocumentValidator* CCollectionDocumentServiceComp::GetDocumentValidator(const QByteArray& typeId) const
{
	int index = GetObjectFactoryIndex(typeId);
