...
void CDesignTokenProcessorsControllerComp::OnComponentCreated()
{
	...
	QString beginInfoString = QStringLiteral("DesignToken started");
	if(m_argumentParserCompPtr.IsValid()){
		...
		if(m_argumentParserCompPtr->IsErrorsIgnoreRequested()){
			SendWarningMessage(0, QStringLiteral("Errors will be ignored!"), __func__);
			beginInfoString += QStringLiteral("\n\t| Errors Ignored");
		}
	...
	QString lockFileName = m_argumentParserCompPtr->GetOutputDirectoryPath() + QStringLiteral("/DesignToken.lock");
	QLockFile lockFile(lockFileName);
	if(!lockFile.lock()){
		SendWarningMessage(0, QStringLiteral("Unable to lock '%1' ").arg(lockFileName), __func__);
	}
