...
class CCrossTenantMessageDbDelegateComp: public imtdb::CSqlDatabaseObjectDelegateCompBase
{
public:
	typedef imtdb::CSqlDatabaseObjectDelegateCompBase BaseClass;

	I_BEGIN_COMPONENT(CCrossTenantMessageDbDelegateComp)
		I_ASSIGN(m_messageFactCompPtr, "MessageFactory", "Factory used for creation of cross-tenant message instances", true, "MessageFactory");
	I_END_COMPONENT
...
private:
	I_FACT(imtauth::ICrossTenantMessageData, m_messageFactCompPtr);
};
