...
class CExternalIdentityDbDelegateComp: public imtdb::CSqlDatabaseObjectDelegateCompBase
{
public:
	typedef imtdb::CSqlDatabaseObjectDelegateCompBase BaseClass;

	I_BEGIN_COMPONENT(CExternalIdentityDbDelegateComp)
		I_ASSIGN(m_identityFactCompPtr, "ExternalIdentity", "Factory used for creation of a new external identity", true, "ExternalIdentity");
	I_END_COMPONENT
...
private:
	I_FACT(imtauth::IExternalIdentity, m_identityFactCompPtr);
};
