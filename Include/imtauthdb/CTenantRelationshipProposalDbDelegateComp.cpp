		...
		const iprm::IParamsSet* /*dataConfigurationPtr*/) const
{
	...
	imtauth::RelationshipProposalInfo info;
	if (record.contains("Id")){
		info.proposalId = imtdb::VariantToByteArray(record.value("Id"));
	}
	if (record.contains("ConnectionId")){
		info.connectionId = imtdb::VariantToByteArray(record.value("ConnectionId"));
	}
	...
	if (record.contains("ProposalType")){
		info.proposalType = static_cast<imtauth::RelationshipProposalType>(record.value("ProposalType").toInt());
	}
	...
	if (record.contains("CounterpartyTenantId")){
		info.counterpartyTenantId = imtdb::VariantToByteArray(record.value("CounterpartyTenantId"));
	}
	...
	if (record.contains("ProposedScope")){
		info.proposedScope = record.value("ProposedScope").toString();
	}
	if (record.contains("ProposedDescription")){
		info.proposedDescription = record.value("ProposedDescription").toString();
	}
	if (record.contains("ProposedValidFrom")){
		info.proposedValidFrom = record.value("ProposedValidFrom").toString();
	}
	...
	if (record.contains("Status")){
		info.status = static_cast<imtauth::RelationshipProposalStatus>(record.value("Status").toInt());
	}
	...
	proposalPtr->SetProposalInfo(info);
		...
		const imtbase::IOperationContext* /*operationContextPtr*/) const
{
	...
	imtauth::RelationshipProposalInfo info = proposalPtr->GetProposalInfo();
	...
	QString existingRelId = NullableSqlText(QString::fromUtf8(info.existingRelationshipId));
	int proposalType = static_cast<int>(info.proposalType);
	QString initiatorTenantId = imtdb::EscapeSql(QString::fromUtf8(info.initiatorTenantId));
	QString counterpartyTenantId = imtdb::EscapeSql(QString::fromUtf8(info.counterpartyTenantId));
	int sourceRole = static_cast<int>(info.proposedSourceRole);
	int targetRole = static_cast<int>(info.proposedTargetRole);
	QString scope = NullableSqlText(info.proposedScope);
	QString description = NullableSqlText(info.proposedDescription);
	QString validFrom = NullableSqlText(info.proposedValidFrom);
	QString validUntil = NullableSqlText(info.proposedValidUntil);
