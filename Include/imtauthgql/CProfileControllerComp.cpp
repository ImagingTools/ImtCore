			...
			QString& errorMessage) const
{
	...
	if (userInfoPtr == nullptr){
		errorMessage = QStringLiteral("Unable to get a profile info. Error: User with ID '%1' does not exists").arg(objectId);
			...
			QString& errorMessage) const
{
	...
	if (userInfoPtr == nullptr){
		errorMessage = QStringLiteral("Unable to set a profile info. Error: User with ID '%1' does not exists").arg(id);
		return sdl::V1_0::imtauth::CSetProfileResponse();
	}
