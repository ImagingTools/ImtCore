...
QJsonObject CRemoteFilterableSelectControllerComp::CreateResponse(const imtgql::CGqlRequest& gqlRequest, QString& errorMessage) const
{
	// If a slave handler is available, delegate the entire request to it
	if (m_slaveRequestHandlerCompPtr.IsValid()){
		if (m_slaveRequestHandlerCompPtr->IsRequestSupported(gqlRequest)){
			return m_slaveRequestHandlerCompPtr->CreateResponse(gqlRequest, errorMessage);
		}
	}

	// Otherwise fall through to the base class which calls OnGetSelectableItems
			...
			QString& errorMessage) const
{
	...
	return GetItemsFromLocalCollection(getSelectableItemsRequest, gqlRequest, errorMessage);
}


sdl::imtbase::FilterableSelect::CGetSelectableItemsPayload CRemoteFilterableSelectControllerComp::GetItemsFromLocalCollection(
			const sdl::imtbase::FilterableSelect::CGetSelectableItemsGqlRequest& getSelectableItemsRequest,
			const ::imtgql::CGqlRequest& gqlRequest,
			QString& errorMessage) const
{
	...
	if (!m_objectCollectionCompPtr.IsValid()){
		errorMessage = QStringLiteral("Neither slave request handler nor local object collection is available");
		SendErrorMessage(0, errorMessage, "CRemoteFilterableSelectControllerComp");
		return response;
	}
