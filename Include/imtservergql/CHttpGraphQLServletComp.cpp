	...
	const imtrest::CHttpRequest& request) const
{
	...
	if (m_gqlContextCacheCompPtr.IsValid()
			&& m_gqlContextCacheCompPtr->TryCreateRequestContext(accessToken, productId, headers, cachedContextPtr)){
		...
		if (m_gqlContextCreatorCompPtr.IsValid()){
			...
			if (m_gqlContextCacheCompPtr.IsValid()){
				m_gqlContextCacheCompPtr->Insert(accessToken, shared.GetPtr());
			}
