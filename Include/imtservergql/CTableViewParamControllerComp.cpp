			...
			QString& errorMessage) const
{
	...
	if (gqlContextPtr == nullptr){
		errorMessage = QStringLiteral("Unable to create response for GraphQL request with ID: '%1'. Invalid GraphQL context.").arg(commandId);
		SendErrorMessage(0, errorMessage, "imtservergql::CTableViewParamControllerComp");
	...
	if (gqlInputParamPtr == nullptr){
		errorMessage = QStringLiteral("Unable to create response for GraphQL request with ID: '%1'. Invalid GraphQL input params.").arg(commandId);
		SendErrorMessage(0, errorMessage, "imtservergql::CTableViewParamControllerComp");
	...
	if (userInfoPtr == nullptr){
		errorMessage = QStringLiteral("Unable to create response for GraphQL request with ID: '%1'. User info from GraphQL context is invalid").arg(commandId);
		SendErrorMessage(0, errorMessage, "imtservergql::CTableViewParamControllerComp");
	...
	if (settingsPtr == nullptr){
		errorMessage = QStringLiteral("Unable to create response for GraphQL request with ID: '%1'. Settings for user with ID: '%2' is invalid.").arg(commandId, userId);
		SendErrorMessage(0, errorMessage, "imtservergql::CTableViewParamControllerComp");
	...
	if (gqlRequest.GetRequestType() == imtgql::IGqlRequest::RT_QUERY){
		...
		if (doc.isNull() || (!doc.isObject() && !doc.isArray())){
			errorMessage = QStringLiteral("Unable to process GraphQL request with ID: '%1'. Invalid table view params json.").arg(commandId);
			SendErrorMessage(0, errorMessage, "imtservergql::CTableViewParamControllerComp");
		...
		imtbase::ITableViewParam* tableViewParamPtr = dynamic_cast<imtbase::ITableViewParam*>(paramSetPtr->GetEditableParameter("TableViewParam"));
		if (tableViewParamPtr == nullptr){
			errorMessage = QStringLiteral("Unable to create response for GraphQL request with ID: '%1'.").arg(commandId);
			SendErrorMessage(0, errorMessage, "imtservergql::CTableViewParamControllerComp");
		...
		if (!m_tableViewParamRepresentationControllerCompPtr->GetDataModelFromRepresentation(tableViewParamRepresentation, *tableViewParamPtr)){
			errorMessage = QStringLiteral("Unable to create response for GraphQL request with ID: '%1'. Error then trying to get a data model from representation model for table view param.").arg(commandId);
			SendErrorMessage(0, errorMessage, "imtservergql::CTableViewParamControllerComp");
		...
		if (m_userSettingsCollectionCompPtr->GetElementIds().contains(userId)){
			if (!m_userSettingsCollectionCompPtr->SetObjectData(userId, *userSettingsPtr.GetPtr())){
				errorMessage = QStringLiteral("Unable to create response for GraphQL request with ID: '%1'. Settings cannot be saved.").arg(commandId);
				SendErrorMessage(0, errorMessage, "imtservergql::CTableViewParamControllerComp");
			...
			if (retVal.isEmpty()){
				errorMessage = QStringLiteral("Unable to create settings for user '%1'. Error: Insert object failed").arg(userId);
