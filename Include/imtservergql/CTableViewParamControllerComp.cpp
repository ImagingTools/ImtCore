			...
			QString& errorMessage) const
{
	...
	if (settingsPtr == nullptr){
		errorMessage = QString("Unable to create response for GraphQL request with ID: '%1'. Settings for user with ID: '%2' is invalid.").arg(commandId, userId);
		SendErrorMessage(0, errorMessage, "imtservergql::CTableViewParamControllerComp");
	...
	if (gqlRequest.GetRequestType() == imtgql::IGqlRequest::RT_QUERY){
		...
		if (doc.isNull() || (!doc.isObject() && !doc.isArray())){
			errorMessage = QString("Unable to process GraphQL request with ID: '%1'. Invalid table view params json.").arg(commandId);
			SendErrorMessage(0, errorMessage, "imtservergql::CTableViewParamControllerComp");
		...
		if (m_userSettingsCollectionCompPtr->GetElementIds().contains(userId)){
			...
			if (retVal.isEmpty()){
				errorMessage = QString("Unable to create settings for user '%1'. Error: Insert object failed").arg(userId);
