...
public:
	...
	virtual imtauth::IUserInfoUniquePtr GetUser(const QByteArray& userId, const QByteArray& productId = QByteArray()) const = 0;
	virtual bool RemoveUser(const QByteArray& userId) = 0;
	virtual QByteArray CreateUser(const QString& userName, const QByteArray& login, const QByteArray& password) = 0;
