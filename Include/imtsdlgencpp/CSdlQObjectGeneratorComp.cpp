...
bool CSdlQObjectGeneratorComp::ProcessHeaderClassFile(QTextStream& stream, const imtsdl::CSdlEntryBase& sdlEntry) const
{
	...
	imtsdl::SdlFieldList fieldList;
	const imtsdl::CSdlUnion* unionPtr = dynamic_cast<const imtsdl::CSdlUnion*>(&sdlEntry);
	const bool isUnion = bool(unionPtr != nullptr);
	...
	if (!isUnion){
		...
		// class Q_PROPERTY
		try{
			const imtsdl::CSdlType& sdlType = dynamic_cast<const imtsdl::CSdlType&> (sdlEntry);
			fieldList = sdlType.GetFields();
		}
		catch (std::bad_cast&){}

		CreatePropertiesForType(stream, fieldList);
		...
		CreateAccessMethodsDefinition(stream, fieldList);
		...
		for (const imtsdl::CSdlField& field: fieldList){
			const QString convertedType = ConvertTypeOrEnumOrUnion(field, m_sdlEnumListCompPtr->GetEnums(false), m_sdlUnionListCompPtr->GetUnions(false));
			FeedStreamHorizontally(stream);
		...
		for (const imtsdl::CSdlField& field: fieldList){
			bool isCustom = false;
			bool isEnum = false;
			const QString convertedType = ConvertTypeOrEnumOrUnion(field, m_sdlEnumListCompPtr->GetEnums(false), m_sdlUnionListCompPtr->GetUnions(false), &isCustom, nullptr, nullptr, &isEnum);
			if (isCustom && !isEnum){
				FeedStreamHorizontally(stream);
	...
	if (isUnion){
		modelDataTypeName = sdlNamespace + QStringLiteral("::") + sdlEntry.GetName();
		modelObjectDataTypeName = sdlNamespace + QStringLiteral("::C") + sdlEntry.GetName() + QStringLiteral("Object");
		objectListClassName = QStringLiteral("C") + sdlEntry.GetName() + QStringLiteral("ObjectList");
		objectListClassNameWithNamespace = sdlNamespace + QStringLiteral("::") + objectListClassName;
	}
...
bool CSdlQObjectGeneratorComp::ProcessSourceClassFile(QTextStream& stream, const imtsdl::CSdlEntryBase& sdlEntry) const
{
	...
	for (const imtsdl::CSdlField& field: fieldList){
		const QString convertedType = ConvertTypeOrEnumOrUnion(field, m_sdlEnumListCompPtr->GetEnums(false), m_sdlUnionListCompPtr->GetUnions(false), &isCustom, nullptr, &isArray, &isEnum, &isUnion);
			...
			FeedStreamHorizontally(stream);
			if (isUnion){
				stream << QStringLiteral("return nullptr; // NOT IMPLEMNTED");
				FeedStream(stream, 1, false);
				stream << '}';
				FeedStream(stream, 2, false);
			...
			std::shared_ptr<imtsdl::CSdlEntryBase> sdlEntryBase = FindEntryByName(field.GetType(), m_sdlTypeListCompPtr->GetSdlTypes(false), m_sdlEnumListCompPtr->GetEnums(false), m_sdlUnionListCompPtr->GetUnions(false));
...
QString CSdlQObjectGeneratorComp::GetQObjectTypeName(const imtsdl::CSdlField& field, bool withPointer) const
{
	...
	const QString convertedType = ConvertTypeOrEnumOrUnion(field, m_sdlEnumListCompPtr->GetEnums(false), m_sdlUnionListCompPtr->GetUnions(false), &isCustom, nullptr, &isArray, &isEnum, &isUnion);
	std::shared_ptr<imtsdl::CSdlEntryBase> sdlEntryBase = FindEntryByName(
				field.GetType(),
...
void CSdlQObjectGeneratorComp::CreatePropertiesForType(QTextStream& stream, const imtsdl::SdlFieldList& fieldList) const
{
	for (const imtsdl::CSdlField& field: fieldList){
		FeedStreamHorizontally(stream);
...
void CSdlQObjectGeneratorComp::CreateAccessMethodsDefinition(QTextStream& stream, const imtsdl::SdlFieldList& fieldList) const
{
	for (const imtsdl::CSdlField& field: fieldList){
		bool isArray = false;
		bool isCustom = false;
