...
void CTaskManagerGuiComp::ShowContextMenu(const QPoint& position, QListView& list)
{
	...
	if (index.isValid()) {
		...
		if (inputsCollectionPtr != nullptr) {
			itemMenu.addAction(m_showInputsManagerCommand.icon(), tr("Acquisition Manager..."), this, SLOT(OnShowInputsManager()));
		}
...
void CTaskManagerGuiComp::OnRenameTask()
{
	...
	if (selectedIds.isEmpty()) {
		// No checkbox-selected tasks: fall back to renaming the currently Qt-selected task
		RenameTask(autoRename);
		return;
	}

	// Rename all checkbox-selected tasks
	imthype::ITaskCollection* objectPtr = BaseClass::GetObjectPtr();
	if (objectPtr == nullptr) {
		return;
	}
	...
	QString newName = QInputDialog::getText(BaseClass::GetWidget(), QObject::tr("Enter new task name"), QObject::tr("Name"), QLineEdit::Normal, currentTaskName).trimmed();
	if (newName.isEmpty()) {
		return;
	}

	const QString baseName = newName;
...
void CTaskManagerGuiComp::OnGuiRetranslate()
{
	...
	m_showInputsManagerCommand.SetVisuals(tr("Show Acquisition &Manager"), tr("Acquisition Manager"), tr("Show editor of the task inputs"), GetIcon(":/Icons/Capture"));
	m_showTriggerTableCommand.SetVisuals(tr("Show &Trigger Table"), tr("Trigger Table"), tr("Show editor of the triggers"), GetIcon(":/Icons/Production"));
}
