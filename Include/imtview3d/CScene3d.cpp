...
QByteArray CScene3d::AddShapeToScene(imtview3d::IScene3dItem* objectPtr, int itemFlags)
{
	if (!objectPtr){
		return QByteArray();
	}
...
void CScene3d::RemoveShapeFromScene(imtview3d::IScene3dItem* objectPtr)
{
	for (Shapes::Iterator i = m_shapes.begin(); i != m_shapes.end(); ++i){
		const ShapeInfoPtr& shapeInfoPtr = i.value();
...
void CScene3d::UpdateBoundingCuboid(const imtview3d::IScene3dItem* objectPtr)
{
