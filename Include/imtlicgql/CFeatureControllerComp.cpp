...
QJsonObject CFeatureControllerComp::GetObjectFromRequest(const imtgql::CGqlRequest& gqlRequest, QString& errorMessage) const
{
	...
	errorMessage = QT_TR_NOOP(QString("Unable to get feature by ID: %1.").arg(objectId));
	SendErrorMessage(0, errorMessage, "Feature controller");
			...
			QString& errorMessage) const
{
	...
	if (!collectionIds.isEmpty()){
		...
		if (objectId != id){
			errorMessage = QT_TR_NOOP(QString("Feature-ID: '%1' already exists. Please rename.")).arg(featureId);
			return false;
		}
	}
