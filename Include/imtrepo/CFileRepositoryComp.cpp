			...
			const QString& targetFilePath) const
{
	...
	if (fileInfo.exists()){
		...
		if (istd::CSystem::FileCopy(fileItemInfo.GetFilePath(), outputFilePath, true)){
			...
			SendErrorMessage(0, QT_TR_NOOP(QStringLiteral("File could not be copied from '%1' to '%2'").arg(fileItemInfo.GetFilePath(), targetFilePath)));
		}
	}
	else{
		SendErrorMessage(0, QT_TR_NOOP(QStringLiteral("File doesn't exist '%1'").arg(fileItemInfo.GetFilePath())));
	}
			...
			const QByteArray& objectId)
{
	...
	if (istd::CSystem::FileCopy(localFilePath, targetFilePath, true)){
		...
		if (indexUpdated){
			...
			SendErrorMessage(0, QT_TR_NOOP(QStringLiteral("File meta info could not be updated")));
...
bool CFileRepositoryComp::RemoveElements(const Ids& elementIds, const imtbase::IOperationContext* /*operationContextPtr*/)
{
	if (!m_documentInfoCollectionCompPtr.IsValid()){
		return false;
	}
	...
	for (const Id& elementId : elementIds){
		...
		if (GetFileInfo(elementId, fileItemInfo)){
			...
			if (!indexUpdated){
				QString targetFilePath = fileItemInfo.GetFilePath();
				SendErrorMessage(0, QT_TR_NOOP(QStringLiteral("File '%1' could not be removed").arg(targetFilePath)));
			}
		}
	}
...
bool CFileRepositoryComp::RemoveElementSet(
...
bool CFileRepositoryComp::GetObjectData(const QByteArray& objectId, DataPtr& dataPtr, const iprm::IParamsSet* /*dataConfigurationPtr*/) const
{
			...
			ilog::IMessageConsumer* messageConsumerPtr) const
{
	...
	while (QFileInfo::exists(newDirPath)){
		newDirPath = QStringLiteral("%1 - %2").arg(targetDirPath, QString::number(++count));
	}
