	...
	if (istd::CSystem::EnsurePathExists(revisionsPath)){
		...
		if (!tempPath.isEmpty()){
			...
			if (sourceFile.exists()){
				...
				SendErrorMessage(0, QObject::tr("Target file '%1' could not be created").arg(targetFilePath));
			}

			tempDir.removeRecursively();
		}
		else{
			SendErrorMessage(0, QObject::tr("Target folder '%1' could not be created").arg(tempPath));
		}
	}
	else{
		SendErrorMessage(0, QObject::tr("Target folder '%1' could not be created").arg(revisionsPath));
	}
			...
			if (QFile(revisionFilePath).exists()){
				...
				if (!tempPath.isEmpty()){
					if (m_compressorCompPtr->DecompressFolder(revisionFilePath, tempPath)){
						...
						SendErrorMessage(0, QObject::tr("Revision '%1' could not be decompressed").arg(revisionFilePath));
					}
					...
					if (ReadItemFile(collectionItem, itemFile)){
						...
						if (QFile(dataFile).exists()){
							...
							if (!savedPath.isEmpty()){
								...
								SendErrorMessage(0, QObject::tr("Item file '%1' could not be saved").arg(newItemFile));
							}
						}
						else{
							SendErrorMessage(0, QObject::tr("Data file '%1' not found").arg(dataFile));
						}
					}
					else{
						SendErrorMessage(0, QObject::tr("Unable read item file '%1'").arg(itemFile));
					}
				}
				else{
					SendErrorMessage(0, QObject::tr("Item file not found in '%1'").arg(workingPath));
				}
			}
			else{
				SendErrorMessage(0, QObject::tr("Unable to uncompress '%1'").arg(sourceFilePath));
			}

			workingDir.removeRecursively();
		}
		else{
			SendErrorMessage(0, QObject::tr("Target folder '%1' could not be created").arg(workingPath));
		}
	}
			...
			ilog::IMessageConsumer* messageConsumerPtr) const
{
	...
	if (newDirPath != targetDirPath){
		QString warning = QObject::tr("Input file name %1 renamed to %2").arg(QDir(targetDirPath).dirName()).arg(QDir(newDirPath).dirName());
