...
bool CFileCollectionCompBase::GetFileInfo(const QByteArray& objectId, IFileCollectionItem& fileCollectionItem) const
{
	...
	if (fileIndex >= 0){
		...
		return fileCollectionItem.CopyFrom(collectionItem);
			...
			const QString& targetFilePath) const
{
	...
	int fileIndex = GetFileIndexById(objectId);
	if (fileIndex >= 0){
		QString filePathInRepository = m_files[fileIndex].GetFilePath();
		...
		if (fileInfo.exists()){
			...
			if (istd::CSystem::FileCopy(filePathInRepository, outputFilePath, true)){
				...
				SendErrorMessage(0, QT_TR_NOOP(QStringLiteral("File could not be copied from '%1' to '%2'").arg(filePathInRepository, targetFilePath)));
			}
		}
		else{
			SendErrorMessage(0, QT_TR_NOOP(QStringLiteral("File doesn't exist '%1'").arg(filePathInRepository)));
		}
	}
			...
			const idoc::IDocumentMetaInfo* collectionItemMetaInfoPtr)
{
	...
	if (IsObjectIdLocked(fileId)){
		SendVerboseMessage(QT_TR_NOOP(QStringLiteral("Object-ID '%1' is locked").arg(proposedObjectId.constData())));

		return emptyId;
	}

	if (IsObjectNameLocked(targetName)){
		SendVerboseMessage(QT_TR_NOOP(QStringLiteral("Object Name '%1' is locked").arg(targetName)));
	...
	if (istd::CSystem::FileCopy(localFilePath, workingFilePath)){
		if (QFile::setPermissions(workingFilePath,
			...
			if (metaInfoCreated){
				CFileCollectionItem collectionItem(
				...
				SendErrorMessage(0, QT_TR_NOOP(QStringLiteral("Meta-information for the file '%1' could not be created").arg(workingFilePath)));
			}
		}
		else{
			SendErrorMessage(0, QT_TR_NOOP(QStringLiteral("Permissions for the file '%1' could not be set").arg(workingFilePath)));
		}
	}
	else{
		SendErrorMessage(0, QT_TR_NOOP(QStringLiteral("Can't copy the file '%1' to working folder").arg(localFilePath)));
	}
			...
			const QByteArray& objectId)
{
	...
	int fileIndex = GetFileIndexById(objectId);
	if (fileIndex < 0){
		SendErrorMessage(0, QT_TR_NOOP(QStringLiteral("No file with the ID '%1' was found in the repository. File could not be updated").arg(objectId.constData())));
	...
	if (!contentsMetaInfoPtr.IsValid()){
		SendErrorMessage(0, QT_TR_NOOP(QStringLiteral("Meta-informations could not be created for'%1'. File could not be updated").arg(localFilePath)));

		return false;
	}

	collectionItem.SetContentsMetaInfo(contentsMetaInfoPtr);

	QFileInfo inputFileInfo(localFilePath);

	QString targetFilePath = collectionItem.GetFilePath();

	if (istd::CSystem::FileCopy(localFilePath, targetFilePath, true)){
		if (!QFile::setPermissions(targetFilePath, QFile::WriteGroup)){
			SendErrorMessage(0, QT_TR_NOOP(QStringLiteral("Permissions for the file '%1' could not be set").arg(targetFilePath)));
		}
		...
		if (!SaveMetaInfo(*collectionItem.GetContentsMetaInfo(), metaInfoFilePath)){
			SendErrorMessage(0, QT_TR_NOOP(QStringLiteral("Meta information of the file '%1' could not be updated").arg(targetFilePath)));
		}
	...
	SendErrorMessage(0, QT_TR_NOOP(QStringLiteral("File '%1' could not be copied to %2").arg(localFilePath, targetFilePath)));
	...
	if (workingDataPtr != nullptr){
		...
		if (persistencePtr != nullptr){
			...
			if (persistencePtr->SaveToFile(*workingDataPtr, tempFilePath) == ifile::IFilePersistence::OS_OK){
				...
				SendVerboseMessage(QT_TR_NOOP(QStringLiteral("File could not be saved into '%1'").arg(tempFilePath)));
			}
		}
	}
			...
			const imtbase::IOperationContext* /*operationContextPtr*/)
{
	...
	if (persistencePtr != nullptr){
		...
		if (persistencePtr->SaveToFile(object, tempFilePath) == ifile::IFilePersistence::OS_OK){
			...
			SendErrorMessage(0, QT_TR_NOOP(QStringLiteral("File could not be saved into '%1'").arg(tempFilePath)));
		}
	}
...
imtbase::IObjectCollectionInfo::Id CFileCollectionCompBase::GetObjectTypeId(const QByteArray& objectId) const
{
	...
	int fileIndex = GetFileIndexById(objectId);
	if (fileIndex >= 0){
		retVal = m_files[fileIndex].GetTypeId();
	}
...
idoc::MetaInfoPtr CFileCollectionCompBase::GetDataMetaInfo(const Id& objectId) const
{
	...
	int fileIndex = GetFileIndexById(objectId);
	if (fileIndex < 0){
		SendVerboseMessage(QT_TR_NOOP(QStringLiteral("Collection item doesn't exist for the given object-ID (%1). Meta-information could not be provided").arg(objectId.constData())), "File Collection");
			...
			ilog::IMessageConsumer* /*logPtr*/) const
{
	...
	for (int fileIndex = offset; fileIndex < objectsCount; ++fileIndex){
		retVal.append(m_files[fileIndex].GetFileId());
	}
...
QVariant CFileCollectionCompBase::GetElementInfo(const QByteArray& elementId, int infoType, ilog::IMessageConsumer* /*logPtr*/) const
{
	...
	if (fileIndex >= 0){
		...
		case EIT_DESCRIPTION:
			return item.GetCollectionMetaInfo().GetMetaInfo(idoc::IDocumentMetaInfo::MIT_DESCRIPTION).toString();
		}
	}
...
idoc::MetaInfoPtr CFileCollectionCompBase::GetElementMetaInfo(const Id& elementId, ilog::IMessageConsumer* /*logPtr*/) const
{
	...
	int fileIndex = GetFileIndexById(elementId);
	if (fileIndex < 0){
		SendVerboseMessage(QT_TR_NOOP(QStringLiteral("Collection item doesn't exist for the given object-ID (%1). Meta-information could not be provided").arg(elementId.constData())), "File Collection");
...
void CFileCollectionCompBase::EnumerateRepositoryItems(QFileInfoList& fileList) const
{
	...
	ifile::CFileListProviderComp::CreateFileList(repositoryRootDir, 0, 2, QStringList() << QStringLiteral("*.%1").arg(GetRepositoryInfo().dataFileSuffix), QDir::Name, fileList);
}
...
QString CFileCollectionCompBase::SaveCollectionItem(const CFileCollectionItem& collectionItem, const QString& dataFilePath) const
{
	...
	SendErrorMessage(0, QT_TR_NOOP(QStringLiteral("Collection item could not be saved into '%1'").arg(itemFilePath)));
	return QString();
}
...
QString CFileCollectionCompBase::GetDataItemFilePath(const CFileCollectionItem& repositoryFile) const
{
	...
	QFileInfo fileInfo(repositoryFile.GetFilePath());
...
void CFileCollectionCompBase::UpdateItemMetaInfo(CFileCollectionItem& item) const
{
	...
	if (item.GetContentsMetaInfo().IsValid()){
		retVal = LoadMetaInfo(*item.GetContentsMetaInfo(), metaInfoFilePath);
	}

	if (!retVal){
		retVal = CreateItemMetaInfoFile(item.GetFilePath(), item.GetTypeId(), metaInfoFilePath);
		if (!retVal){
			SendErrorMessage(0, QT_TR_NOOP(QStringLiteral("Meta-information for the file '%1' could not be created. Meta-information could not be provided").arg(metaInfoFilePath)), "File Collection");
		}
	}
}
...
void CFileCollectionCompBase::ReadItem(Files& filesPtr, const QString& itemFilePath) const
{
	...
	if (!ReadItemFile(fileItem, itemFilePath)){
		SendErrorMessage(0, QT_TR_NOOP(QStringLiteral("Collection item could not be loaded from '%1'").arg(itemFilePath)));
		return;
	}

	QFileInfo repositoryFileInfo(fileItem.GetFilePath());
	if (repositoryFileInfo.exists()){
		if (!fileItem.GetContentsMetaInfo().IsValid()){
			idoc::MetaInfoPtr retVal;

			QByteArray typeId = fileItem.GetTypeId();
			if (m_metaInfoCreatorMap.contains(typeId)){
				m_metaInfoCreatorMap[typeId]->CreateMetaInfo(nullptr, typeId, retVal);
			}
			...
			fileItem.SetContentsMetaInfo(retVal);
		}

		if (fileItem.GetContentsMetaInfo().IsValid()){
			LoadMetaInfo(*fileItem.GetContentsMetaInfo(), GetMetaInfoFilePath(fileItem));

			filesPtr.push_back(fileItem);
		}
	}
	else{
		SendErrorMessage(0, QT_TR_NOOP(QStringLiteral("File '%1' doesn't exist. Collection item was automatically removed").arg(fileItem.GetFilePath())));
	}
}
...
bool CFileCollectionCompBase::IsObjectIdUsed(const QByteArray& objectId)
{
	...
	for (int i = 0; i < m_files.count(); ++i){
		if (m_files[i].GetFileId() == objectId){
			return true;
		}
	}
...
void CFileCollectionCompBase::OnComponentCreated()
{
	...
	if (!path.isEmpty()){
		if (!istd::CSystem::EnsurePathExists(path)){
			SendCriticalMessage(0, QT_TR_NOOP(QStringLiteral("Root folder for the file collection could not be created in '%1'").arg(path)));
		}
	}
