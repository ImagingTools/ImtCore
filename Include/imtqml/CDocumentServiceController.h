...
class IDocumentServiceBridge;


/**
	\brief Transport-agnostic C++ analog of the QML
	\c DocumentServiceBase / \c GqlBasedCollectionDocumentService.
	...
	\c IDocumentServiceBridge implementation resolved at runtime — by
	default to \c CDocumentServiceBridge (in-process, on top of
	\c imtdoc::IDocumentService).

	\note Bridge resolution currently uses
	\c CDocumentServiceBridge::Instance() to keep parity with the
	existing \c CGqlClientBridge lookup convention. The bridge is
	intentionally not exposed to QML; users just write
	\c DocumentServiceController \c { collectionId: "X" } and the QML
	code remains transport-agnostic.

	The class is registered to QML by \c CStaticQmlTypeRegistratorComp
	under \c com.imtcore.imtqml 1.0 as \c DocumentServiceController.
*/
class CDocumentServiceController: public QObject
{
	Q_OBJECT
	Q_PROPERTY(QString collectionId READ GetCollectionId WRITE SetCollectionId NOTIFY collectionIdChanged)
	Q_PROPERTY(QObject* documentManagerActiveView READ getDocumentServiceActiveView WRITE setDocumentServiceActiveView NOTIFY documentManagerActiveViewChanged)
...
public:
	...
	explicit CDocumentServiceController(QObject* parent = nullptr);
	~CDocumentServiceController() override;
...
public Q_SLOTS:
	...
	QObject* getDocumentServiceActiveView() const;
	void setDocumentServiceActiveView(QObject* view);
...
Q_SIGNALS:
	...
	// --- Mirrored from DocumentServiceBase ---
...
private:
	...
	IDocumentServiceBridge* ResolveBridge() const;
...
Q_DECLARE_METATYPE(imtqml::CDocumentServiceController*)
