...
QByteArray CScene3d::AddShapeToScene(eagle::IScene3dItem* objectPtr, int itemFlags)
{
	if (!objectPtr){
		return QByteArray();
	}
...
void CScene3d::RemoveShapeFromScene(eagle::IScene3dItem* objectPtr)
{
	for (Shapes::Iterator i = m_shapes.begin(); i != m_shapes.end(); ++i){
		const ShapeInfoPtr& shapeInfoPtr = i.value();
...
void CScene3d::UpdateBoundingCuboid(const eagle::IScene3dItem* objectPtr)
{
