...
{
	QByteArray id = m_managerPtr->CreateInvitation(m_invitedByUserId, m_userId, m_tenantId, "member");
	...
	QCOMPARE(inv->roleId, QByteArray("member"));
...
{
	QByteArray id1 = m_managerPtr->CreateInvitation(m_invitedByUserId, m_userId, m_tenantId, "member");
	QVERIFY(!id1.isEmpty());

	// Second pending invitation for same user+tenant should fail
	QByteArray id2 = m_managerPtr->CreateInvitation(m_invitedByUserId, m_userId, m_tenantId, "admin");
	QVERIFY(id2.isEmpty());
}
...
{
	QByteArray invId = m_managerPtr->CreateInvitation(m_invitedByUserId, m_userId, m_tenantId, "member");
...
void CTenantInvitationManagerTest::testGetInvitationsByTenant_FilterByStatus()
{
	...
	m_managerPtr->CreateInvitation(m_invitedByUserId, m_userId, m_tenantId, "member");
	QByteArray inv2 = m_managerPtr->CreateInvitation(m_invitedByUserId, user2, m_tenantId, "admin");
	m_managerPtr->CreateInvitation(m_invitedByUserId, user3, m_tenantId, "viewer");
...
void CTenantInvitationManagerTest::testGetInvitationsByUser_AllStatuses()
{
	...
	m_managerPtr->CreateInvitation(m_invitedByUserId, m_userId, m_tenantId, "member");
	m_managerPtr->CreateInvitation(m_invitedByUserId, m_userId, tenant2, "admin");
