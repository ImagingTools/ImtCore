...
if not defined IMTCOREDIR (
    ...
    for %%A in ("%APP_ROOT%\..\ImtCore" "%APP_ROOT%\..\..\ImtCore" "%APP_ROOT%\..\..\..\ImtCore") do (
        if not defined IMTCOREDIR if exist "%%~fA\Tests\Docker\Scripts\run-tests-windows-core.bat" set IMTCOREDIR=%%~fA
    )
    if not defined IMTCOREDIR (
