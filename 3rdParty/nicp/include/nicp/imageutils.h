...
namespace nicp {
   ...
   */
	NICP_API void DepthImage_scale(DepthImage &dest, const DepthImage &src, int step, float maxDepthCov = 0.01f);
   ...
   *  (for example in case it's 2 the size of the image will be half the size of the original one).
   */
	NICP_API void RGBImage_scale(RGBImage &dest, const RGBImage &src, int step);
   ...
   */
	NICP_API void DepthImage_convert_32FC1_to_16UC1(cv::Mat &dest, const cv::Mat &src, float scale = 1000.0f);
   ...
   */
	NICP_API void DepthImage_convert_16UC1_to_32FC1(cv::Mat &dest, const cv::Mat &src, float scale = 0.001f);
